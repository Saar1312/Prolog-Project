...
conseguirLista(Nodos,Cantidad,EtiquetasNodo,Lista,RestoEtiquetas):-
	...
	concat([Hijo],Nodos,NewNodos),
	conseguirLista(NewNodos,NewCantidad,NewEtiquetasNodo,Lista,RestoEtiquetas).

conseguirArista(Padre,[],ListaAristas):- ListaAristas=[].
conseguirArista(Padre,[LN|LNS],ListaAristas):-
	EA is abs(Padre-LN),
...
construido(N,R,Padre,[LNumHijos|Nhs],Aristas):-
	rango(Padre,[Ea|Eas],1),
	construido2(EtiquetasNodo,N,PadreN),
	Aristas = [arista(Ea,Arbol)|AS]. % = o is?

construido2(EtiquetasNodo,N,UltimoNodo,):-
	rango(N,EtiquetasNodo,Algo),
...
aristaValida(NumNodos,N1,E,N2):-
	NumAristas is NumNodos - 1,
	E =:= abs(N1-N2).%,etiquetaValida(NumAristas,E).*/
%---------------------------------------------------------------
etiquetamiento(esq(Le),Arbol):-
	evaluar(esq(Le),N,R),
	construidoI(Le,N,R,Arbol).